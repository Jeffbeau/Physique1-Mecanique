...
\item[$\star\star$] \textbf{7.} Un patrouilleur des garde-côtes part de sa base, parcourt $\SI{40}{NM}$ vers le nord à $\SI{20}{\knots}$, puis revient à sa base à $\SI{15}{\knots}$.
\begin{enumerate}[label=\alph*)]
...
\item[$\star\star$] \textbf{8.} Un vraquier effectue un trajet de $\SI{1200}{NM}$. Il navigue à $\SI{12}{\knots}$ pendant les 20 premières heures, puis à $\SI{15}{\knots}$ pour le reste du trajet. Quelle est sa vitesse scalaire moyenne?

\item[$\star\star\star$] \textbf{9.} \textbf{Question piège!} Un navire parcourt la \textit{première moitié} d'un trajet à $\SI{10}{\knots}$ et la \textit{seconde moitié} à $\SI{20}{\knots}$. 
\begin{enumerate}[label=\alph*)]
    \item Quelle est sa vitesse scalaire moyenne? 
    \item Expliquez pourquoi ce n'est \textbf{pas} $\SI{15}{\knots}$.
\end{enumerate}

\item[$\star\star\star$] \textbf{10.} Un autre navire parcourt la \textit{première moitié du temps} à $\SI{10}{\knots}$ et la \textit{seconde moitié du temps} à $\SI{20}{\knots}$. Quelle est sa vitesse scalaire moyenne? Comparez avec l'exercice précédent.
...
\begin{enumerate}[resume]
\item[$\star$] \textbf{20.} Un porte-conteneurs passe de $\SI{5}{\knots}$ à $\SI{18}{\knots}$ en $\SI{12}{min}$. Calculez son accélération moyenne en m/s².

\item[$\star$] \textbf{21.} Un TGV roulant à $\SI{320}{km/h}$ freine et s'arrête en $\SI{4}{min}$. Quelle est sa décélération moyenne en m/s²?

\item[$\star\star$] \textbf{22.} Un traversier part du repos et accélère à $\SI{0,08}{m/s^2}$ jusqu'à atteindre $\SI{12}{\knots}$.
\begin{enumerate}[label=\alph*)]
    \item Combien de temps dure la phase d'accélération?
    \item Quelle distance parcourt-il pendant cette phase?
\end{enumerate}

\item[$\star\star$] \textbf{23.} Un cargo navigue à $\SI{15}{\knots}$. Le capitaine ordonne le freinage avec $a = \SI{-0,006}{m/s^2}$.
\begin{enumerate}[label=\alph*)]
...
\item[$\star\star\star$] \textbf{25.} Un navire doit s'arrêter exactement à un quai situé à $\SI{800}{m}$. Il arrive à $\SI{8}{\knots}$.
\begin{enumerate}[label=\alph*)]
...
\item[$\star\star\star$] \textbf{27.} Deux navires se trouvent à $\SI{3}{km}$ l'un de l'autre et naviguent l'un vers l'autre. Le navire A voyage à vitesse constante de $\SI{8}{\knots}$ tandis que le navire B part du repos et accélère à $\SI{0,01}{m/s^2}$.
\begin{enumerate}[label=\alph*)]
...
\begin{enumerate}[resume]
\item[$\star$] \textbf{34.} Un navire navigue à $\SI{18}{\knots}$ avec un cap de $40\si{\degree}$ nord de l'est.
\begin{enumerate}[label=\alph*)]
    \item Quelle est la composante est-ouest de sa vitesse?
    \item Quelle est la composante nord-sud de sa vitesse?
\end{enumerate}

\item[$\star\star$] \textbf{35.} Un lance-amarre projette une ligne à $\SI{25}{m/s}$ avec un angle de $40\si{\degree}$ depuis une hauteur de $\SI{6}{m}$.
\begin{enumerate}[label=\alph*)]
    \item Calculez les composantes $v_{0x}$ et $v_{0y}$.
    \item Quelle est la portée horizontale?
\end{enumerate}

\item[$\star\star$] \textbf{36.} Un joueur de baseball frappe une balle à $\SI{35}{m/s}$ avec un angle de $30\si{\degree}$ au-dessus de l'horizontale, depuis une hauteur de $\SI{1}{m}$.
\begin{enumerate}[label=\alph*)]
...
\item[$\star\star\star$] \textbf{38.} Une fusée éclairante est lancée à $\SI{40}{m/s}$ avec un angle de $60\si{\degree}$ depuis le niveau de l'eau.
\begin{enumerate}[label=\alph*)]
...
\begin{enumerate}[resume]
\item[$\star$] \textbf{40.} Convertissez : a) $270\si{\degree}$ en radians \quad b) $\frac{3\pi}{4}$~rad en degrés \quad c) $\SI{90}{RPM}$ en rad/s
...
Un cargo de haute mer arrive au port de Montréal. À $\SI{2}{km}$ du quai, il navigue à $\SI{10}{\knots}$. Le pilote du port ordonne une première phase de freinage avec $a_1 = \SI{-0,005}{m/s^2}$ jusqu'à atteindre $\SI{3}{\knots}$, puis une seconde phase avec $a_2 = \SI{-0,008}{m/s^2}$ jusqu'à l'arrêt complet.
...
Un marin tombe d'un navire qui se déplace à $\SI{14}{\knots}$. L'équipage met $\SI{45}{s}$ avant de réagir (temps de réaction), puis le navire freine avec une décélération de $\SI{0,15}{m/s^2}$ jusqu'à l'arrêt.
...
Deux navires se font face dans un chenal. Le navire A ($\SI{14}{\knots}$ vers l'est) et le navire B ($\SI{10}{\knots}$ vers l'ouest) sont séparés de $\SI{1500}{m}$. Les deux capitaines freinent simultanément avec des décélérations de $a_A = \SI{-0,008}{m/s^2}$ et $a_B = \SI{-0,005}{m/s^2}$.
...
\begin{enumerate}[label=\alph*)]
    ...
    \item Quelle décélération minimale \textbf{identique} les deux navires auraient-ils dû appliquer pour éviter la collision?
\end{enumerate}

\item[\textbf{S9.}] \textbf{Lancer du poids}

Un athlète lance un poids de $\SI{7,26}{kg}$ avec une vitesse de $\SI{13}{m/s}$ à un angle de $40\si{\degree}$ au-dessus de l'horizontale. Le poids quitte sa main à une hauteur de $\SI{2}{m}$.
...
Un cargo quitte Sept-Îles à 6h00 pour Rimouski ($\SI{180}{NM}$). Il navigue d'abord à $\SI{12}{\knots}$ pendant 8 heures, puis accélère à $\SI{0,002}{m/s^2}$ jusqu'à atteindre $\SI{16}{\knots}$, vitesse qu'il maintient jusqu'à destination.
...
\begin{itemize}
    ...
    \item Le navire B est à $\SI{5}{km}$ à l'est, navigue vers l'ouest à $\SI{8}{\knots}$ et maintient cette vitesse
\end{itemize}
...
\begin{itemize}
    \item Phase 1 : Accélération de $\SI{0}{n\oe{}ud}$ à $\SI{6}{\knots}$ avec $a_1 = \SI{0,01}{m/s^2}$
    \item Phase 2 : Vitesse constante de $\SI{6}{\knots}$ pendant $\SI{15}{min}$
    \item Phase 3 : Accélération de $\SI{6}{\knots}$ à $\SI{12}{\knots}$ avec $a_3 = \SI{0,008}{m/s^2}$
    \item Phase 4 : Vitesse constante de $\SI{12}{\knots}$ jusqu'à destination
\end{itemize}
...
\begin{itemize}
    \item Le navire S navigue vers l'est à $\SI{15}{\knots}$ (vitesse constante)
    \item Le navire P est immobile mais peut atteindre $\SI{25}{\knots}$ avec une accélération de $\SI{0,03}{m/s^2}$
\end{itemize}
...
\begin{itemize}
    ...
    \item Au centre : courant maximal de $\SI{3}{\knots}$ vers l'est
    \item Au bord nord : courant nul
\end{itemize}

Le navire part du bord sud et navigue à $\SI{10}{\knots}$ (vitesse surface) cap au nord.
...
Un cargo navigue vers l'est à vitesse constante de $\SI{12}{\knots}$. Un garde-côte, situé $\SI{800}{m}$ au sud de la trajectoire du cargo et exactement à la même longitude (vis-à-vis de sa position actuelle), doit l'intercepter. Le garde-côte peut accélérer à $\SI{0,03}{m/s^2}$ jusqu'à une vitesse maximale de $\SI{25}{\knots}$.
...
\begin{enumerate}[label=\alph*)]
    ...
    \item Pour un angle de $30\si{\degree}$ est du nord, calculez le temps nécessaire au garde-côte pour atteindre la trajectoire du cargo (la ligne est-ouest à $y = \SI{800}{m}$).
...
\textbf{4.} a) vrai (égalité si ligne droite sans demi-tour) \quad b) Vrai \quad c) Faux (ne dépend que des positions initiale et finale)

\medskip
\textbf{Vitesse moyenne et scalaire}

\textbf{5.} $\SI{27,3}{km/h}$ ; $\SI{7,58}{m/s}$ ; $\SI{14,7}{\knots}$

\textbf{6.} $\SI{15}{\knots}$ ; $\SI{7,72}{m/s}$

\textbf{7.} a) $\SI{4,67}{h}$ \quad b) $\SI{0}{\knots}$ \quad c) $\SI{17,1}{\knots}$

\textbf{8.} $\SI{14,3}{\knots}$

\textbf{9.} a) $\SI{13,3}{\knots}$ \quad b) Le temps passé à chaque vitesse n'est pas égal

\textbf{10.} $\SI{15}{\knots}$ (moyenne arithmétique car temps égaux)

\textbf{11.} a) $\SI{1,0}{h} = \SI{60}{min}$ \quad b) $\SI{90}{km}$ de la gare A

\textbf{12.} a) $\approx$ 9h43 \quad b) $\SI{171}{km}$ de Montréal
...
\textbf{15.} a) $\SI{1}{m/s/min} = \SI{0,017}{m/s^2}$ \quad b) Entre 4 et 10 min \quad c) $\SI{2400}{m}$

\textbf{16.} Le signe dépend du choix de l'axe; une pente négative indique un mouvement vers les $x$ décroissants

\textbf{17.} a) MRU, accélération, MRU, freinage \quad b) $\SI{0,0042}{m/s^2}$ \quad c) $\SI{-0,011}{m/s^2}$ \quad d) $\SI{3240}{m}$

\textbf{18.} a) $\SI{4,8}{m/s}$ \quad b) $\SI{4,8}{m/s}$ \quad c) $\SI{3,6}{m/s}$

\textbf{19.} a) Accélération, MRU, freinage \quad b) $\SI{5}{m/s}$ \quad c) Oui (pentes nulles aux extrémités)
...
\textbf{27.} a) $x_A = 4{,}12t$ ; $x_B = 3000 - 0{,}005t^2$ \quad b) $\SI{466}{s} \approx \SI{7,8}{min}$ \quad c) $\SI{1920}{m}$ de A \quad d) $\SI{4,7}{m/s} \approx \SI{9,1}{\knots}$
...
\textbf{30.} a) $\SI{1,01}{s}$ \quad b) $\SI{12,9}{m/s}$

\textbf{31.} a) $\SI{36,9}{m}$ \quad b) $\SI{5,3}{s}$

\textbf{32.} a) $\SI{5,27}{m}$ \quad b) $\SI{1,85}{s}$

\textbf{33.} La balle A (lâchée) a la plus grande vitesse; la balle B est à son apogée ($v_B = 0$) au point de croisement

\medskip
\textbf{Mouvement en 2D et projectile}

\textbf{34.} a) $\SI{13,8}{\knots}$ \quad b) $\SI{11,6}{\knots}$
...
\textbf{38.} a) $\SI{61,2}{m}$ \quad b) $\SI{141}{m}$ \quad c) $45\si{\degree}$
...
\textbf{40.} a) $\frac{3\pi}{2} \approx \SI{4,71}{rad}$ \quad b) $135\si{\degree}$ \quad c) $\SI{9,42}{rad/s}$
...
\textbf{44.} a) $\SI{25,1}{m/s} \approx \SI{90}{km/h}$ \quad b) $\SI{-21,1}{rad/s^2}$ \quad c) $26,5$ tours
...
\textbf{S1.} a) $\SI{2400}{m}$ \quad b) $\SI{148}{m}$ \quad c) Non, s'arrête à $\SI{548}{m}$ après le quai; aurait dû commencer à $\SI{2548}{m}$

\textbf{S2.} a) $\SI{324}{m}$ \quad b) $\SI{173}{m}$ \quad c) $\SI{497}{m}$ \quad d) $\SI{545}{m}$

\textbf{S3.} a) $\SI{71,4}{m}$ \quad b) $v = \SI{37,5}{m/s}$ à $48,2\si{\degree}$ sous l'horizontale \quad c) Réduit de $\SI{5,7}{m}$ (larguer à $\SI{65,7}{m}$) \quad d) $\SI{59,8}{m}$

\textbf{S4.} a) $\SI{0,628}{rad/s^2}$ \quad b) $\SI{0,785}{m/s}$ \quad c) $\SI{1,96}{m}$ \quad d) $\SI{1,65}{s}$

\textbf{S5.} a) $\SI{24,4}{m}$ \quad b) $\SI{52}{m}$ \quad c) Oui, s'arrête à $\SI{3,6}{m}$ de l'obstacle

\textbf{S6.} a) $\SI{4,9}{m/s}$ \quad b) $\SI{1,23}{m}$ \quad c) $\SI{0,33}{s}$ \quad d) $\SI{12,97}{m}$ (s'arrête à environ $\SI{13}{m}$)

\textbf{S7.} a) $\SI{66,7}{m}$ \quad b) $\SI{11,1}{s}$ \quad c) $t \approx \SI{30}{s}$ \quad d) Police : $\SI{1007}{m}$ ; Fuyard : $\SI{1007}{m}$

\textbf{S8.} a) $A$: $t = \SI{900}{s}$, $d = \SI{3241}{m}$ ; $B$: $t = \SI{1029}{s}$, $d = \SI{2646}{m}$ \quad b) Non, collision \quad c) Position $\approx \SI{872}{m}$ de A; $v_A \approx \SI{6,2}{m/s}$, $v_B \approx \SI{4,5}{m/s}$ \quad d) $a \geq \SI{0,026}{m/s^2}$

\textbf{S9.} a) $\SI{5,55}{m}$ \quad b) $\SI{17,8}{m}$ \quad c) $\approx 42\si{\degree}$ (légèrement moins que 45\si{\degree} car lancé en hauteur) \quad d) Portée augmente de $\approx 21\%$ (proportionnel à $v^2$)
...
\textbf{D1.} a) $\Delta y = \SI{+5}{m}$ (tire vers le bas) \quad b) $\theta_{min} \approx 28\si{\degree}$ \quad c) $t \approx \SI{1,4}{s}$ \quad d) $\theta_{min}$ préférable (trajectoire plus tendue, moins affectée par le vent) \quad e) Portée réduite de $\SI{0,7}{m}$

\textbf{D2.} a) $x_A = \frac{1}{2}(0,02)t^2$; $x_B = 5000 - 4,12t$ \quad b) $t = \SI{650}{s}$, $x = \SI{4220}{m}$ \quad c) $\SI{13}{m/s} = \SI{25,3}{\knots}$ \quad d) $v_{A/B} = \SI{17,1}{m/s}$ vers l'est (A s'approche rapidement de B) \quad e) Commencer à freiner à $t = \SI{217}{s}$

\textbf{D3.} a) Phase 1: $\SI{309}{s}$, $\SI{476}{m}$; Phase 2: $\SI{900}{s}$, $\SI{2778}{m}$; Phase 3: $\SI{386}{s}$, $\SI{1736}{m}$ \quad b) $\SI{5010}{m}$ restants, $\SI{811}{s}$ \quad c) $\SI{2406}{s} \approx \SI{40}{min}$ \quad d) Voir graphique \quad e) $v_{moy} = \SI{15,0}{km/h} = \SI{8,1}{\knots}$ (inférieure à $\SI{9}{\knots}$, moyenne arithmétique)
...
\textbf{D5.} a) S se déplace; quand P atteint la position initiale de S, celui-ci est déjà ailleurs \quad b) $x_P = v_P(t)\sin\theta \cdot t$; $y_P = v_P(t)\cos\theta \cdot t$; $x_S = 7,72t$; $y_S = 8000$ \quad c) $x_P = x_S$ et $y_P = y_S$ \quad d) $\theta \approx 50\si{\degree}$ est du nord, $t \approx \SI{620}{s}$ \quad e) $\approx \SI{6,2}{km}$
