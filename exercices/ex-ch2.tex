...
\begin{enumerate}[label=\alph*)]
    ...
    \item Une caisse de \SI{100}{kg} est immobile sur une rampe de chargement inclinée à $30\si{\degree}$, retenue par le frottement statique.
    \item Un conteneur est retenu en l'air par deux câbles : le premier fait un angle de $20\si{\degree}$ avec la verticale et le second fait un angle de $45\si{\degree}$ avec la verticale.
\end{enumerate}
...
\begin{enumerate}[label=\alph*)]
    ...
    \item Trois forces : poids $\vect{F}_g$ (vers le bas) et deux tensions $\vect{T}_1$ (le long du premier câble, à $20\si{\degree}$ de la verticale) et $\vect{T}_2$ (le long du second câble, à $45\si{\degree}$ de la verticale).
\end{enumerate}
...
\begin{itemize}
    \item le câble AB est fixé au plafond et fait un angle de $40\si{\degree}$ avec l'horizontale;
...
\item[$\star\star$] \textbf{6. (Maritime)} Un conteneur de \SI{2500}{kg} est soulevé par la grue d'un navire. Le câble de la grue fait un angle de $20\si{\degree}$ avec la verticale. Une amarre horizontale est attachée au conteneur pour l'empêcher de balancer.
...
\item[$\star\star$] \textbf{8. (Maritime)} Une caisse de \SI{500}{kg} est immobile sur la passerelle de chargement d'un navire, inclinée à $25\si{\degree}$ par rapport à l'horizontale.
...
\item[$\star\star$] \textbf{12. (Maritime)} Un treuil tire une caisse de \SI{80}{kg} sur un quai à l'aide d'un câble faisant un angle de $25\si{\degree}$ au-dessus de l'horizontale. Le coefficient de frottement cinétique entre la caisse et le quai est $\mu_c = 0{,}30$. La tension dans le câble est de \SI{350}{N}.
...
\item[$\star\star$] \textbf{13. (Maritime)} Une palette de \SI{200}{kg} est relâchée du repos au sommet d'une rampe de chargement inclinée à $30\si{\degree}$ et longue de \SI{4}{m}. Le coefficient de frottement cinétique est $\mu_c = 0{,}20$.
...
\item[$\star\star$] \textbf{14. (Maritime)} Un baril de \SI{40}{kg} part du repos et descend une rampe inclinée à $25\si{\degree}$, longue de \SI{2}{m}, avec un coefficient de frottement cinétique $\mu_c = 0{,}15$. Au bas de la rampe, le baril continue sur le plancher horizontal de la cale, où le coefficient de frottement cinétique est $\mu_c = 0{,}20$. À quelle distance du bas de la rampe le baril s'immobilise-t-il?
...
\item[$\star\star\star$] \textbf{15. (Maritime)} Lors du chargement d'un navire, une caisse de \SI{50}{kg} est posée sur une rampe inclinée à $30\si{\degree}$ avec un coefficient de frottement cinétique $\mu_c = 0{,}15$. Elle est reliée par un câble passant par une poulie sans frottement au sommet de la rampe à un contrepoids de \SI{40}{kg} suspendu dans le vide. Le système est relâché du repos.
...
\begin{enumerate}[label=\alph*)]
    ...
    \item En combien de temps le navire effectue-t-il un demi-tour ($180\si{\degree}$)?
...
% =============================================================================
% SECTION E — SYNTHÈSE ET INTÉGRATION (version révisée)
% =============================================================================
% Changements par rapport à la version précédente :
%   - Ex 21 : Remplacé. Nouveau scénario de chargement en 4 étapes chaînées
%             (rampe + statique grue + levage + déplacement sur pont).
%   - Ex 22 : Remplacé. Scénario de stabilité en mer combinant mouvement
%             circulaire, gîte (plan incliné) et frottement (sec vs mouillé).
%   - Ex 23 : Remplacé. Manutention d'un moteur en 3 étapes indépendantes
%             mais dans un récit cohérent (quai + descente + roulis).
%   - L'ancien ex 23 (estimation distance de freinage) est déplacé vers
%     la section C (dynamique) où il est mieux classé (★★, un seul concept).
% =============================================================================

\subsection*{E — Synthèse et intégration}

\begin{remarque}[title=Ce que signifie « synthèse »]
Ces exercices combinent \textbf{plusieurs concepts du chapitre} dans un scénario maritime réaliste. Contrairement aux exercices précédents, vous devrez :
\begin{itemize}
    \item identifier \textbf{vous-même} quels principes appliquer à chaque étape;
    \item \textbf{réutiliser} les résultats d'une étape dans les suivantes;
    \item porter un \textbf{jugement} sur la sécurité d'une situation.
\end{itemize}
\end{remarque}

\begin{enumerate}[resume]

% ---- EXERCICE 21 — Chargement par rampe et grue ★★★ -------------------------
\item[$\star\star\star$] \textbf{21. (Maritime — Chargement par rampe et grue)} Une caisse de \SI{500}{kg} doit être chargée dans la cale d'un navire. L'opération se déroule en quatre étapes.

\textbf{Étape 1 — Montée de la rampe.} La caisse est tirée \textit{depuis le repos} jusqu'au sommet d'une rampe de \SI{4}{m} inclinée à $30\si{\degree}$ par un câble parallèle à la rampe. Le coefficient de frottement cinétique entre la caisse et la rampe est $\mu_c = 0{,}20$. La caisse doit atteindre le sommet avec une vitesse de \SI{1,5}{m/s}.

\textbf{Étape 2 — Suspension par la grue.} Au sommet, le câble de la rampe est détaché et la grue du navire saisit la caisse. Le câble de la grue fait un angle de $25\si{\degree}$ avec la verticale. Un hauban horizontal empêche la caisse de balancer. La caisse est en équilibre.

\textbf{Étape 3 — Levage.} La grue soulève la caisse \textit{verticalement} avec une accélération de \SI{0,50}{m/s^2}.

\textbf{Étape 4 — Déplacement sur le pont.} La caisse est déposée sur le pont du navire. Un chariot élévateur la pousse horizontalement avec une force de \SI{1800}{N}. Le coefficient de frottement cinétique entre la caisse et le pont est $\mu_c = 0{,}25$.

\begin{enumerate}[label=\alph*)]
    \item Calculez l'accélération requise sur la rampe (étape 1), puis la tension dans le câble.
    \item Calculez la tension dans le câble de la grue et dans le hauban (étape 2).
    \item Calculez la tension dans le câble de la grue pendant le levage (étape 3). Comparez avec le poids de la caisse.
    \item Calculez l'accélération de la caisse sur le pont (étape 4) et sa vitesse après \SI{4}{m}.
\end{enumerate}

\textit{Réponses :}

\textit{a) Cinématique : $a = v_f^2 / (2d) = \SI{0,281}{m/s^2}$. Sur le plan incliné :}

\textit{$T = m(a + g\sin 30\si{\degree} + \mu_c \, g\cos 30\si{\degree}) = \SI{500}{}(0{,}281 + 4{,}905 + 1{,}699) = \SI{3440}{N} \approx \SI{3,44}{kN}$}

\textit{b) $T_{\text{câble}} = mg / \cos 25\si{\degree} = \SI{5,41}{kN}$ \quad $T_{\text{hauban}} = T_{\text{câble}} \sin 25\si{\degree} = \SI{2,29}{kN}$}

\textit{c) $T = m(g + a) = \SI{500}{}(9{,}81 + 0{,}50) = \SI{5160}{N} \approx \SI{5,16}{kN}$. C'est $105\%$ du poids : le câble doit supporter plus que le poids de la caisse pour l'accélérer vers le haut.}

\textit{d) $f_c = \mu_c \, mg = \SI{1230}{N}$; \quad $F_{\text{net}} = 1800 - 1230 = \SI{574}{N}$; \quad $a = \SI{1,15}{m/s^2}$; \quad $v = \sqrt{2 \times 1{,}15 \times 4} = \SI{3,03}{m/s}$}

\bigskip

% ---- EXERCICE 22 — Stabilité de la cargaison en mer ★★★ ---------------------
\item[$\star\star\star$] \textbf{22. (Maritime — Stabilité de la cargaison en mer)} Un conteneur de \SI{10\,000}{kg} est posé sur le pont d'un vraquier.

\textbf{Situation 1 — Virage en eau calme.} Le navire effectue un virage de rayon \SI{500}{m} à une vitesse de 12 nœuds. Le coefficient de frottement statique entre le conteneur et le pont est $\mu_s = 0{,}40$.

\textbf{Situation 2 — Tempête.} Le navire est frappé par une vague et gîte (s'incline) de $15\si{\degree}$. Le pont devient un plan incliné.

\textbf{Situation 3 — Pont mouillé.} De l'eau de mer inonde le pont pendant la tempête. Le coefficient de frottement statique chute à $\mu_s = 0{,}20$ et le coefficient de frottement cinétique est $\mu_c = 0{,}12$.

\begin{enumerate}[label=\alph*)]
    \item Calculez l'accélération centripète du navire en virage (situation 1). Le conteneur risque-t-il de glisser? Justifiez par un calcul.
    \item Tracez le DCL du conteneur sur le pont incliné à $15\si{\degree}$ (situation 2). Calculez le coefficient de frottement statique \textbf{minimum} requis pour que le conteneur ne glisse pas.
    \item Avec le pont mouillé (situation 3), le conteneur glisse-t-il? Si oui, calculez son accélération et sa vitesse après avoir glissé de \SI{2}{m}.
    \item Quel est l'angle de gîte maximal sécuritaire avec un pont mouillé ($\mu_s = 0{,}20$)?
\end{enumerate}

\textit{Réponses :}

\textit{a) $v = 12 \times 0{,}514 = \SI{6,17}{m/s}$; \quad $a_c = v^2/R = \SI{0,0761}{m/s^2}$. Force centripète sur le conteneur : $F_c = \SI{761}{N}$. Frottement maximal : $f_{s,\text{max}} = \mu_s \, mg = \SI{39\,200}{N}$. Le conteneur ne risque absolument pas de glisser en virage (facteur de sécurité $\approx 52$).}

\textit{b) $\mu_{s,\text{min}} = \tan 15\si{\degree} = 0{,}268$. Avec un pont sec ($\mu_s = 0{,}40$), le conteneur tient.}

\textit{c) Pont mouillé : $\mu_s = 0{,}20 < 0{,}268$ $\Rightarrow$ \textbf{le conteneur glisse!}}

\textit{$a = g(\sin 15\si{\degree} - \mu_c \cos 15\si{\degree}) = 9{,}81(0{,}259 - 0{,}116) = \SI{1,40}{m/s^2}$}

\textit{$v = \sqrt{2 \times 1{,}40 \times 2} = \SI{2,37}{m/s}$ (\SI{8,5}{km/h}) — un conteneur de 10 tonnes en mouvement est extrêmement dangereux.}

\textit{d) $\theta_{\text{max}} = \arctan(\mu_s) = \arctan(0{,}20) = 11{,}3\si{\degree}$}

\begin{remarque}[title=Pertinence maritime]
Ce problème illustre pourquoi l'arrimage de la cargaison est l'une des responsabilités les plus critiques de l'officier de navigation. Le vrai danger ne vient pas des virages (les accélérations centripètes sont négligeables aux vitesses de navigation), mais de la gîte causée par les vagues et le vent. Un pont mouillé ou glacé réduit drastiquement les marges de sécurité. Des conteneurs mal arrimés qui glissent en tempête ont causé de nombreux accidents maritimes.
\end{remarque}

\bigskip

% ---- EXERCICE 23 — Manutention d'un moteur ★★★ ------------------------------
\item[$\star\star\star$] \textbf{23. (Maritime — Manutention d'un moteur)} Un moteur diesel de \SI{600}{kg} doit être installé dans la salle des machines d'un navire. L'opération se déroule en trois étapes.

\textbf{Étape 1 — Transport sur le quai.} Le moteur est posé sur un chariot à roues (frottement de roulement négligeable). Un débardeur pousse le chariot horizontalement avec une force de \SI{200}{N}. Simultanément, un câble attaché au treuil du navire tire le chariot avec une tension de \SI{800}{N} à un angle de $35\si{\degree}$ au-dessus de l'horizontale.

\textbf{Étape 2 — Descente dans la salle des machines.} Le moteur est détaché du chariot et suspendu à une corde qui passe par une poulie. L'opérateur doit le descendre de \SI{3}{m}. La corde a une résistance maximale de \SI{7000}{N}.

\textbf{Étape 3 — Roulis du navire.} Une fois posé sur une étagère de la salle des machines, le navire subit du roulis et l'étagère s'incline momentanément de $10\si{\degree}$. Le coefficient de frottement statique entre le moteur et l'étagère est $\mu_s = 0{,}15$ et le coefficient cinétique est $\mu_c = 0{,}10$.

\begin{enumerate}[label=\alph*)]
    \item Calculez l'accélération du chariot sur le quai (étape 1). N'oubliez pas de considérer la composante verticale du câble.
    \item Le moteur descend dans la salle des machines. Quelle est la décélération maximale que l'opérateur peut appliquer sans que la corde casse (étape 2)? À quelle vitesse maximale peut-il amorcer la descente s'il doit s'arrêter en \SI{3}{m}?
    \item Lors du roulis (étape 3), le moteur glisse-t-il? Si oui, calculez son accélération et sa vitesse après avoir glissé de \SI{1,5}{m}. Quel serait l'angle de roulis maximal sécuritaire?
\end{enumerate}

\textit{Réponses :}

\textit{a) Horizontale : $F_x = 200 + 800 \cos 35\si{\degree} = \SI{855}{N}$. Verticale : la composante $T \sin 35\si{\degree} = \SI{459}{N}$ allège le chariot mais celui-ci reste au sol ($N = mg - T\sin 35\si{\degree} = \SI{5430}{N} > 0$). \quad $a = F_x / m = 855/600 = \SI{1,43}{m/s^2}$}

\textit{b) Descente avec freinage : $T - mg = ma$ (en prenant vers le haut comme positif). \quad $a_{\text{max}} = T_{\text{max}}/m - g = 7000/600 - 9{,}81 = \SI{1,86}{m/s^2}$. \quad Vitesse initiale max : $v = \sqrt{2 \times 1{,}86 \times 3} = \SI{3,34}{m/s}$}

\textit{c) $\tan 10\si{\degree} = 0{,}176 > \mu_s = 0{,}15$ $\Rightarrow$ \textbf{le moteur glisse!} \quad $a = g(\sin 10\si{\degree} - \mu_c \cos 10\si{\degree}) = \SI{0,737}{m/s^2}$. \quad Après \SI{1,5}{m} : $v = \SI{1,49}{m/s}$ (\SI{5,4}{km/h}). \quad Angle maximal sécuritaire : $\theta_{\text{max}} = \arctan(0{,}15) = 8{,}5\si{\degree}$.}
