...
\begin{remarque}[title=Pour aller plus loin]
Les applications détaillées de la force centripète (virage de navire, inclinaison en virage, mouvement circulaire vertical) sont présentées à la section~\ref{sec:circulaire}.
\end{remarque}

% =============================================================================
\subsection{La force de rappel (loi de Hooke)}
\label{subsec:force-rappel}
% =============================================================================

Lorsqu'on comprime ou étire un ressort (ou tout objet élastique), celui-ci exerce une force qui tend à le ramener à sa position naturelle. C'est la \textbf{force de rappel}.

\begin{definition}[title=Loi de Hooke]
La force exercée par un ressort est proportionnelle à son allongement (ou sa compression) :
\[
    F_{\text{ressort}} = -k \Delta x
\]
où $k$ est la \textbf{constante de raideur} du ressort (\si{N/m}) et $\Delta x$ est le déplacement par rapport à la position d'équilibre.

Le signe négatif indique que la force s'oppose toujours au déplacement : si on étire le ressort ($\Delta x > 0$), la force tire vers l'arrière ($F < 0$), et inversement.
\end{definition}

\begin{exemple}{Ressort d'amortissement d'un quai}{ressort-quai}
Un navire accoste et comprime un amortisseur de quai (pare-battage à ressort) de $\Delta x = \SI{15}{cm}$. La constante de raideur de l'amortisseur est $k = \SI{8000}{N/m}$. Quelle force l'amortisseur exerce-t-il sur le navire?

\tcblower
La compression vaut $\Delta x = \SI{0{,}15}{m}$. En module :
\[
    F = k |\Delta x| = 8000 \times 0{,}15 = \boxed{\SI{1200}{N}}
\]
La force est dirigée vers l'extérieur du quai (elle repousse le navire), ce qui est cohérent : le ressort comprimé pousse pour revenir à sa longueur naturelle.
\end{exemple}
...
\hline
\textbf{Force de rappel} & $F = -k\Delta x$ & Oppose le déplacement par rapport à la position d'équilibre \\
\hline
\end{tabular}
\end{center}
...
\begin{enumerate}
    ...
    \item Une \textbf{caisse} tirée sur le sol par une corde faisant un angle de 30\si{\degree} avec l'horizontale.
    
    \item Un \textbf{bloc} immobile sur un plan incliné à 25\si{\degree}.
...
\begin{enumerate}
    ...
    \item Poids (vers le bas), Normale (vers le haut), Tension (30\si{\degree} vers le haut-avant), Frottement (opposé au mouvement)
