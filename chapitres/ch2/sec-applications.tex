...
Un moteur diesel auxiliaire de masse $m = 200$~kg est suspendu dans la salle des machines d'un navire par deux câbles. Le câble de bâbord fait un angle de 30\si{\degree} avec l'horizontale, celui de tribord fait un angle de 45\si{\degree} avec l'horizontale. Déterminer les tensions $T_1$ et $T_2$ dans chaque câble.
...
\begin{tikzpicture}[scale=1.2]
    ...
    \begin{scope}[xshift=-4cm]
        ...
        % Câbles - géométrie correcte pour 30\si{\degree} et 45\si{\degree}
        % T1: angle 30\si{\degree} → tan(30\si{\degree})=0.577, si Δy=1.5, Δx=1.5/0.577=2.6
        \draw[thick] (-2.6,3) -- (0,1.5) node[midway, above left] {$T_1$};
        % T2: angle 45\si{\degree} → tan(45\si{\degree})=1, si Δy=1.5, Δx=1.5
        ...
        \node at (-2.1,2.2) {\small $30\si{\degree}$};
        
        \draw[dashed, gray] (1.5,3) -- (1.5,1.5);
        \draw (1.5,2.4) arc (270:225:0.6);
        \node at (1.1,2.2) {\small $45\si{\degree}$};
    ...
    \begin{scope}[xshift=3cm]
        ...
        % Tension T1 (30\si{\degree} au-dessus de l'horizontale, vers la gauche)
        % cos(30\si{\degree})=0.866, sin(30\si{\degree})=0.5
        ...
        \node at (-0.75,0.25) {\small $30\si{\degree}$};
        
        % Tension T2 (45\si{\degree} au-dessus de l'horizontale, vers la droite)
        % cos(45\si{\degree})=0.707, sin(45\si{\degree})=0.707
        \draw[->, very thick, blue] (0,0) -- ({1.3*cos(45)},{1.3*sin(45)}) node[above right] {$\vect{T}_2$};
        \draw (0.5,0) arc (0:45:0.5);
        \node at (0.7,0.3) {\small $45\si{\degree}$};
...
\begin{itemize}
    ...
    \item Tension $T_1$ : inconnue (vers le haut-gauche, à 30\si{\degree} de l'horizontale)
    \item Tension $T_2$ : inconnue (vers le haut-droite, à 45\si{\degree} de l'horizontale)
\end{itemize}
...
\begin{align*}
    T_{1x} &= -T_1\cos 30\si{\degree} & T_{1y} &= T_1\sin 30\si{\degree} \\
    T_{2x} &= T_2\cos 45\si{\degree} & T_{2y} &= T_2\sin 45\si{\degree}
\end{align*}

Selon $x$ :
\begin{equation}
    -T_1\cos 30\si{\degree} + T_2\cos 45\si{\degree} = 0 \tag{1}
\end{equation}

Selon $y$ :
\begin{equation}
    T_1\sin 30\si{\degree} + T_2\sin 45\si{\degree} - mg = 0 \tag{2}
\end{equation}

\textbf{\underline{Étape 4 — ALGÈBRE}}

De l'équation (1) :
\[ T_2 = T_1 \frac{\cos 30\si{\degree}}{\cos 45\si{\degree}} = T_1 \times \frac{0{,}866}{0{,}707} = 1{,}225 \, T_1 \]

Substituons dans l'équation (2) :
\[ T_1\sin 30\si{\degree} + 1{,}225 \, T_1 \sin 45\si{\degree} = mg \]
...
\textbf{Vérification :} $T_2 > T_1$ car le câble de droite est plus vertical (45\si{\degree} > 30\si{\degree}), il supporte donc une plus grande partie du poids. 
...
Un feu de navigation de masse $m = 12$~kg est suspendu entre deux mâts d'un navire par deux câbles. Le câble de bâbord fait un angle de 40\si{\degree} avec l'horizontale, celui de tribord fait un angle de 55\si{\degree} avec l'horizontale.
...
Un bloc de masse $m = 15$~kg est posé sur un plan incliné à $\theta = 25\si{\degree}$ par rapport à l'horizontale. Le bloc reste immobile. Le coefficient de frottement statique entre le bloc et le plan est $\mu_s = 0{,}5$.
...
\begin{tikzpicture}[scale=1.0]
    ...
    \begin{scope}[xshift=3cm]
        ...
        % θ=25\si{\degree}: Fg fait angle θ avec l'axe -y
        % Fg_x = mg·sin(25\si{\degree}) ≈ 0.42·mg (positif, vers bas pente)
        % Fg_y = -mg·cos(25\si{\degree}) ≈ -0.91·mg (négatif, vers la pente)
...
\[ f_s = mg\sin\theta = 15 \times 9{,}81 \times \sin 25\si{\degree} = 147{,}2 \times 0{,}423 \]
\[ \boxed{f_s = 62{,}2~\text{N}} \]

Vérifions que le bloc peut rester en équilibre. La normale vaut :
\[ N = mg\cos\theta = 15 \times 9{,}81 \times \cos 25\si{\degree} = 133{,}4~\text{N} \]
...
\[ \boxed{\theta_{\max} = 26{,}6\si{\degree}} \]
...
Une caisse de marchandise de 80~kg est posée sur une rampe de chargement d'un cargo, inclinée à 22\si{\degree}. Le coefficient de frottement statique est $\mu_s = 0{,}45$.
...
Sur un quai, une caisse A de masse $m_A = 80$~kg est posée sur une rampe inclinée à 30\si{\degree} (sans frottement). Elle est reliée par un câble passant par une poulie à un contrepoids B suspendu au-dessus de l'eau.
...
\tcblower
\textit{Réponse :} $m_B = m_A \sin 30\si{\degree} = 40$~kg
\end{pratiqueautonome}
...
\[ \theta = \arctan\left(\frac{a_y}{a_x}\right) = \arctan\left(\frac{0{,}0075}{0{,}01}\right) = \arctan(0{,}75) = \boxed{36{,}9\si{\degree}~\text{nord de l'est}} \]
...
Trois câbles tirent sur un anneau au sol pour positionner une bouée de navigation. Le câble 1 exerce une force de 500~N vers le nord. Le câble 2 exerce une force de 400~N à 60\si{\degree} est du nord. Le câble 3 exerce une force de 300~N à 45\si{\degree} ouest du sud.
...
\tcblower
\textit{Réponse :} $F_{\text{rés}} = 538$~N à $30{,}5\si{\degree}$ est du nord
\end{pratiqueautonome}
...
Un skieur de masse $m = 75$~kg descend une pente inclinée à $\theta = 20\si{\degree}$ par rapport à l'horizontale. Le coefficient de frottement cinétique entre les skis et la neige est $\mu_c = 0{,}08$. Calculer l'accélération du skieur.
...
\begin{tikzpicture}[scale=1.0]
    ...
    \begin{scope}[xshift=3cm]
        ...
        % θ=20\si{\degree}: sin(20\si{\degree})=0.342, cos(20\si{\degree})=0.940
        % Fg = (mg·sin20\si{\degree}, -mg·cos20\si{\degree}) en repère incliné
...
\[ a_x = 9{,}81 \times (\sin 20\si{\degree} - 0{,}08 \times \cos 20\si{\degree}) \]
...
\textbf{Vérification :} 
\begin{itemize}
    \item Sans frottement ($\mu_c = 0$), on aurait $a = g\sin 20\si{\degree} = 3{,}35$~m/s². Le frottement réduit bien l'accélération.
...
Un conteneur de 500~kg glisse vers le bas d'une rampe de déchargement inclinée à 15\si{\degree}. Le coefficient de frottement cinétique est $\mu_c = 0{,}12$.
...
\tcblower
\textit{Réponses :} 1) $a = 1{,}40$~m/s² \quad 2) $v = 4{,}10$~m/s \quad 3) $\theta = 6{,}8\si{\degree}$
\end{pratiqueautonome}
