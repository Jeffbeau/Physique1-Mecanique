...
\begin{enumerate}
    \item Un bloc glisse sur un plan incliné à 40\si{\degree} vers la droite.
    \item Un pendule est à 25\si{\degree} de la verticale, tiré vers la gauche.
    \item Une voiture accélère horizontalement vers la droite.
    \item Une balle de masse $m$ est lancée à 60\si{\degree} au-dessus de l'horizontale (analysez au point de lancer).
\end{enumerate}
...
\begin{enumerate}
    \item Axes : $x$ // pente (+ vers le bas), $y$ $\perp$ pente (+ vers l'extérieur). $F_{gx} = mg\sin 40\si{\degree}$, $F_{gy} = -mg\cos 40\si{\degree}$
    \item Axes : $x$ // fil (+ vers le bas), $y$ $\perp$ fil. $F_{gx} = mg\cos 25\si{\degree}$, $F_{gy} = -mg\sin 25\si{\degree}$
