...
\begin{tikzpicture}[scale=1.0]
    ...
    \begin{scope}[xshift=2.5cm]
        ...
        \node[red, left] at (-0.1, 0.2) {$\vect{F}_g$};
        
        % Normale
        \draw[-{Stealth[length=3mm]}, very thick, green!60!black] (0, 0.8) -- (0, 1.9);
        \node[green!60!black, left] at (-0.1, 1.4) {$\vect{N}$};
        ...
        \node[blue, above] at (-0.5, 0.9) {$\vect{f}$};
        
        % Angle de la tension
        \draw[thick] (0.5, 0.8) arc (0:30:0.5);
        \node at (0.7, 1.0) {\small $\theta$};
    \end{scope}
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.8]
    ...
    % Poids (vers le bas)
    \draw[-{Stealth[length=3mm]}, very thick, red] (0, 2) -- (0, 0);
    \node[red, left] at (-0.1, 1) {$\vect{F}_g = mg$};
    
    % T1 (fait un angle alpha avec l'horizontale)
    \draw[-{Stealth[length=3mm]}, very thick, orange] (0, 0) -- ({2.5*cos(30)}, {2.5*sin(30)});
    \node[orange, below] at (1.2, 0.5) {$\vect{T}_1$};
    
    % T2 (revient au point de départ)
    \draw[-{Stealth[length=3mm]}, very thick, green!60!black] ({2.5*cos(30)}, {2.5*sin(30)}) -- (0, 2);
    \node[green!60!black, above right] at (1.4, 1.8) {$\vect{T}_2$};
    
    % Angles dans le triangle
    % Angle au sommet bas (entre Fg et T1) = 90° + alpha
    \draw[thick] (0, 0.5) arc (90:30:0.5);
    \node at (0.4, 0.7) {\small $90°{-}\alpha$};
    
    % Angle au sommet droit (entre T1 et T2) = 180° - alpha - beta
    \draw[thick] ({2.5*cos(30) - 0.4*cos(30+60)}, {2.5*sin(30) + 0.4*sin(30+60)}) arc (120:180-15:0.4);
    
    % Angle au sommet haut (entre T2 et Fg) = 90° - beta
    \draw[thick] (0, 1.5) arc (270:270+45:0.5);
    \node at (0.35, 1.3) {\small $90°{-}\beta$};
    
    % Annotation
    \node[text width=5cm, align=left] at (5.5, 1) {
