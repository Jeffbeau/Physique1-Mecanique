...
% -----------------------------------------------------------------------------
\subsubsection{Virage horizontal (non bancé)}
% -----------------------------------------------------------------------------
...
\begin{remarque}[title=D'où vient la force centripète sur un navire en virage?]
Quand un navire effectue un virage, la force centripète est fournie par \textbf{deux mécanismes} :
...
\begin{tikzpicture}[scale=0.9]
    ...
    \begin{scope}[xshift=0cm]
        ...
        \draw[dashed, gray] (0, 0) arc (180:270:3);
        \fill (3, -3) circle (2pt);
        \node[below right] at (3, -3) {\small Centre};
        
        % Navire (simplifié, vue de dessus)
        \begin{scope}[rotate=-45, shift={(2.12, 0)}]
            \draw[thick, fill=gray!30] (-0.8, -0.25) -- (0.8, -0.25) -- (1.0, 0) -- (0.8, 0.25) -- (-0.8, 0.25) -- (-0.9, 0) -- cycle;
            % Safran
            \draw[thick, fill=blue!40, rotate=20] (-0.9, -0.15) rectangle (-1.1, 0.15);
        \end{scope}
        
        % Force du safran
        \draw[-{Stealth[length=3mm]}, very thick, blue] (1.3, -1.7) -- (1.8, -2.2);
        \node[blue, below, font=\small] at (1.8, -2.3) {Safran};
        
        % Pression latérale de l'eau
        \draw[-{Stealth[length=3mm]}, very thick, orange] (1.6, -1.2) -- (2.3, -1.9);
        \node[orange, right, font=\small, text width=2cm] at (2.4, -1.5) {Pression latérale};
        
        % Vitesse tangentielle
        \draw[-{Stealth[length=3mm]}, thick, purple] (1.5, -1.5) -- (0.5, -2.5);
        \node[purple, below left] at (0.7, -2.3) {$\vec{v}$};
        
        % Force centripète résultante
        \draw[-{Stealth[length=4mm]}, very thick, red] (1.5, -1.5) -- (2.5, -2.5);
        \node[red, right] at (2.6, -2.5) {$\vec{F}_c$};
    \end{scope}
    
    % Légende
    \node[right, text width=6cm, font=\small] at (5, -1.5) {
        \textbf{1. Le safran} dévie l'eau, créant une force latérale sur la poupe.\\[0.5em]
        \textbf{2. La coque} reçoit une pression latérale de l'eau lorsqu'elle « pousse » contre le virage.\\[0.5em]
        Ces deux forces combinées fournissent la force centripète nécessaire.
    };
\end{tikzpicture}
\end{center}

\textbf{Conséquence pratique :} Plus le virage est serré (petit $r$) ou rapide (grand $v$), plus la force centripète requise est grande. Un navire lourdement chargé nécessite un rayon de virage plus grand ou une vitesse réduite pour effectuer la même manœuvre en toute sécurité.
\end{remarque}