...
\begin{remarque}
Pour un projectile lanc\'e et retombant au m\^eme niveau, la port\'ee est maximale lorsque l'angle de lancement est de $45°$.
\end{remarque}
...
\begin{exemple}{Ballon de soccer (exemple terrestre)}{}
Un joueur botte un ballon avec une vitesse initiale de $\SI{20}{m/s}$ \`a un angle de $30°$ par rapport au sol.
...
\textbf{Donn\'ees :}
\begin{itemize}
    \item $v_0 = \SI{20}{m/s}$, $\theta = 30°$
    \item $v_{0x} = 20 \cos 30° = \SI{17,3}{m/s}$
    \item $v_{0y} = 20 \sin 30° = \SI{10}{m/s}$
\end{itemize}
...
\begin{exemple}{Lance-amarre (exemple maritime)}{}
Un marin utilise un lance-amarre pour envoyer une ligne vers un quai. L'appareil lance le projectile à $\SI{25}{m/s}$ avec un angle de $40°$. Le marin se trouve sur le pont à $\SI{6}{m}$ au-dessus du niveau du quai.

Quelle est la portée horizontale du tir?

\textbf{Données :}
\begin{itemize}
    \item $v_0 = \SI{25}{m/s}$, $\theta = 40°$
    \item $y_i = \SI{6}{m}$, $y_f = \SI{0}{m}$ (niveau du quai)
    \item $v_{0x} = 25 \cos 40° = \SI{19,2}{m/s}$
    \item $v_{0y} = 25 \sin 40° = \SI{16,1}{m/s}$
\end{itemize}
...
Un lance-amarre tire une ligne avec une vitesse initiale de $\SI{30}{m/s}$ à un angle de $40°$ au-dessus de l'horizontale, depuis une hauteur de $\SI{4}{m}$ au-dessus de l'eau.
...
\begin{exemple}{Homme à la mer depuis un navire en mouvement}{}
...
\textbf{Analyse :} Au moment de la chute, le marin a la même vitesse horizontale que le navire. C'est un projectile lancé horizontalement ($\theta = 0°$).
...
\textbf{Temps de chute :}
\[ \Delta t = \sqrt{\frac{2h}{g}} = \sqrt{\frac{2 \times 10}{9,81}} = \SI{1,43}{s} \]
...
\begin{exemple}{Probl\`eme inverse : trouver l'angle de lancement}{}
...
\[ 2\theta = \arcsin(0,545) = 33,0° \quad \Rightarrow \quad \theta = 16,5° \]

\textbf{Attention!} L'\'equation $\sin(2\theta) = 0,545$ a \textbf{deux solutions} entre $0°$ et $90°$ :
\begin{itemize}
    \item $2\theta = 33,0°$ $\Rightarrow$ $\theta_1 = 16,5°$ (tir tendu)
    \item $2\theta = 180° - 33,0° = 147°$ $\Rightarrow$ $\theta_2 = 73,5°$ (tir en cloche)
\end{itemize}
...
% Trajectoire basse (16.5°)
\draw[very thick, blue, domain=0:7, samples=50] plot (\x, {0.42*\x - 0.03*\x*\x});
\draw[thick, blue, ->] (0,0) -- (1.4,0.42) node[above left] {\small $\theta_1 = 16,5°$};

% Trajectoire haute (73.5°)
\draw[very thick, red, domain=0:7, samples=50] plot (\x, {2.38*\x - 0.17*\x*\x});
\draw[thick, red, ->] (0,0) -- (0.42,1.4) node[above left] {\small $\theta_2 = 73,5°$};
...
\textbf{R\'eponse :} Deux angles sont possibles : $\theta_1 = 16,5°$ ou $\theta_2 = 73,5°$.

\begin{remarque}[title=Choix pratique]
En pratique, le tir tendu ($16,5°$) est souvent pr\'ef\'er\'e car :
\begin{itemize}
...
\begin{exemple}{Port\'ee maximale impossible}{}
...
\textbf{Port\'ee maximale :} Elle est atteinte quand $\sin(2\theta) = 1$, soit $\theta = 45°$ :
...
Un cas particulier important est le projectile lancé \textbf{horizontalement} (angle $\theta = 0°$).
...
Le temps de chute ne dépend que de la hauteur :
\[ \Delta t = \sqrt{\frac{2h}{g}} \]

Et la portée est :
\[ \text{Portée} = v_0 \times \sqrt{\frac{2h}{g}} \]

\begin{exemple}{Conteneur tombant d'une grue}{}
...
\[ \Delta t = \sqrt{\frac{2 \times 15}{9,81}} = \SI{1,75}{s} \]

\[ \Delta x = 2 \times 1,75 = \SI{3,5}{m} \]
