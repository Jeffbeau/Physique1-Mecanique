...
Un cargo navigue vers l'est \`a $\SI{12}{\knots}$ (vitesse surface). Le courant porte vers l'ouest \`a $\SI{3}{\knots}$.
...
\begin{itemize}
    \item $v_{surface} = +\SI{12}{\knots}$ (vers l'est)
    \item $v_{courant} = -\SI{3}{\knots}$ (vers l'ouest)
\end{itemize}

\[ v_{fond} = v_{surface} + v_{courant} = 12 + (-3) = \SI{9}{\knots} \text{ vers l'est} \]

Le navire avance effectivement \`a $\SI{9}{\knots}$ par rapport au fond.
\end{exemple}

\begin{exemple}{Travers\'ee avec courant lat\'eral --- Décomposition vectorielle}{traversee-courant-lateral}
Un traversier doit rejoindre un quai situ\'e exactement au nord, \`a $\SI{2}{km}$. Sa vitesse propre (surface) est de $\SI{10}{\knots}$, mais un courant de $\SI{3}{\knots}$ porte vers l'est.
...
\[ v_{fond} = v_{fond,y} = v_{surface}\cos\theta = 10 \times \cos(17{,}5^\circ) = \boxed{\SI{9{,}5}{\knots}} \]
