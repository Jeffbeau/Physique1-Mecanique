...
\hline
$\SI{1}{\knots}$ (nd ou kn) & $= \SI{1,852}{km/h} = \SI{0,5144}{m/s}$ \\
\hline
\end{tabular}
\end{center}

Le n\oe{}ud est l'unit\'e de vitesse en navigation : $\SI{1}{\knots} = \SI{1}{mille nautique/heure}$.
\end{remarque}
...
\[ v_{scalaire} = \SI{1,08}{m/s} \times \frac{\SI{1}{\knots}}{\SI{0,5144}{m/s}} = \SI{2,1}{\knots} \]
...
Vitesse moyenne :
\[ v_{moy} = \frac{\Delta x}{\Delta t} = \frac{\SI{+180}{NM}}{\SI{12}{h}} = \SI{+15}{\knots} \]
...
Calculez la vitesse moyenne du cargo en km/h et en noeuds.

\espaceresolution[5cm]
\reponsepratique{$v_{moy} = \SI{13}{km/h} \approx \SI{7,0}{\knots}$}
...
Un traversier effectue l'aller-retour entre deux rives d'un fleuve s\'epar\'ees de $\SI{2}{km}$. \`A l'aller (avec le courant), il navigue \`a $\SI{12}{\knots}$. Au retour (contre le courant), sa vitesse tombe \`a $\SI{8}{\knots}$.

\textbf{Conversion des vitesses en km/h :}
\begin{align*}
v_{aller} &= \SI{12}{\knots} \times \frac{\SI{1,852}{km/h}}{\SI{1}{\knots}} = \SI{22,2}{km/h} \\[0.3cm]
v_{retour} &= \SI{8}{\knots} \times \frac{\SI{1,852}{km/h}}{\SI{1}{\knots}} = \SI{14,8}{km/h}
\end{align*}
...
\textbf{Vitesse scalaire moyenne :}
\[ v_{scalaire} = \frac{d}{\Delta t} = \frac{\SI{4}{km}}{\SI{0,225}{h}} = \SI{17,8}{km/h} = \SI{17,8}{km/h} \times \frac{\SI{1}{\knots}}{\SI{1,852}{km/h}} = \SI{9,6}{\knots} \]

\begin{attention}
La vitesse scalaire moyenne ($\SI{9,6}{\knots}$) n'est \textbf{pas} la moyenne arithm\'etique des deux vitesses : $\dfrac{12 + 8}{2} = \SI{10}{\knots}$.
...
\begin{pratiqueautonome}
Un remorqueur effectue un aller-retour entre deux quais séparés de $\SI{3}{km}$. À l'aller, il navigue à $\SI{10}{\knots}$. Au retour, sa vitesse est de $\SI{6}{\knots}$.
...
\reponsepratique{a) $\Delta t \approx \SI{19,4}{min}$ \quad b) $v_{scalaire} \approx \SI{7,5}{\knots}$ (et non $\SI{8}{\knots}$!)}
\end{pratiqueautonome}
...
\begin{itemize}
    \item \`A $t = \SI{0}{s}$ : $v = \SI{0}{\knots}$
    \item \`A $t = \SI{60}{s}$ : $v = \SI{3}{\knots}$
    \item \`A $t = \SI{120}{s}$ : $v = \SI{6}{\knots}$
    \item \`A $t = \SI{180}{s}$ : $v = \SI{8}{\knots}$
\end{itemize}
...
En navigation, on distingue deux vitesses fondamentales qui peuvent différer significativement :

\begin{definition}[title=Vitesse surface (STW) et vitesse fond (SOG)]
\begin{itemize}
    \item La \textbf{vitesse surface} (en anglais : \textit{Speed Through Water}, \textbf{STW}) est la vitesse du navire \textbf{par rapport à l'eau} environnante. C'est ce que mesure le \textbf{loch} (speedomètre du navire).

    \item La \textbf{vitesse fond} (en anglais : \textit{Speed Over Ground}, \textbf{SOG}) est la vitesse du navire \textbf{par rapport au fond marin} (ou à la Terre). C'est ce qu'affiche le \textbf{GPS}.
\end{itemize}

Ces deux vitesses diff\`erent lorsqu'il y a du \textbf{courant}. La relation fondamentale est \textbf{vectorielle} :
\begin{equationimportante}
\begin{equation}
\vec{v}_{fond} = \vec{v}_{surface} + \vec{v}_{courant}
\end{equation}
\end{equationimportante}
ou en notation alternative : $\text{SOG} = \text{STW} + \text{courant}$ (vectoriellement)
\end{definition}

\begin{remarque}[title=Importance pratique en navigation]
\begin{itemize}
    \item La \textbf{vitesse surface (STW)} détermine le comportement hydrodynamique du navire (gouvernabilité, résistance de l'eau, consommation de carburant).
    \item La \textbf{vitesse fond (SOG)} détermine le temps réel pour atteindre une destination et doit être utilisée pour calculer l'\textbf{heure d'arrivée estimée} (ETA).
    \item Un navire peut afficher $\SI{10}{\knots}$ au loch mais n'avancer que de $\SI{7}{\knots}$ par rapport au fond s'il navigue contre un courant de $\SI{3}{\knots}$.
\end{itemize}
\end{remarque}

\begin{exemple}{Effet du courant sur la navigation}{}
Un cargo affiche $\SI{12}{\knots}$ au loch (vitesse surface). Il navigue dans le fleuve Saint-Laurent où le courant est de $\SI{2}{\knots}$.

\textbf{Cas 1 : Courant favorable (même sens que le navire)}
\[ v_{fond} = 12 + 2 = \SI{14}{\knots} \]

\textbf{Cas 2 : Courant contraire (sens opposé)}
\[ v_{fond} = 12 - 2 = \SI{10}{\knots} \]

Dans le premier cas, le navire arrivera \textbf{plus tôt} que prévu. Dans le second cas, \textbf{plus tard}. C'est la vitesse fond (GPS) qui détermine l'heure d'arrivée réelle.
\end{exemple}
