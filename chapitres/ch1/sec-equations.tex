...
\begin{tikzpicture}[scale=0.85]
    ...
    % Distance initiale
    \draw[<->] (0.8,-0.4) -- (3.5,-0.4) node[midway, below] {$d = \SI{2}{km}$};
    
    % Vitesse du cargo (MRU)
    \draw[vecteur] (3.5,0.25) -- (5.5,0.25) node[midway, above] {$\vec v_C = \SI{10}{nd}$};
    
    % Accélération du patrouilleur (MRUA)
    \draw[vecteur vert] (0.8,-0.15) -- (2.3,-0.15) node[midway, below] {$\vec a_P$};
    
    % Étiquettes
    \node[above] at (0.8,0.75) {Patrouilleur};
    \node[above] at (3.5,1.0) {Cargo};
    \node[below] at (0.8,0) {$v_{P,i} = 0$};
\end{tikzpicture}
\end{center}
