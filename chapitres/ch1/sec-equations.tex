...
Un cargo navigue en ligne droite \`a $\SI{12}{\knots}$ ($\SI{6,17}{m/s}$). Quelle distance parcourt-il en 3 heures?
...
Un cargo quitte le port A vers l'est \`a $v_1 = \SI{12}{\knots}$. Au m\^eme moment, un p\'etrolier quitte le port B, situ\'e \`a $\SI{40}{NM}$ \`a l'est de A, et se dirige vers l'ouest \`a $v_2 = \SI{8}{\knots}$.
...
\end{tikzpicture}
\end{center}
Un cargo navigue \`a $\SI{12}{\knots}$ lorsque le capitaine ordonne l'arr\^et des machines. Le navire d\'ec\'el\`ere \`a $\SI{0,005}{m/s^2}$. Quelle distance parcourt-il avant de s'immobiliser?

\textbf{Donn\'ees :}
\begin{itemize}
    \item $v_i = \SI{12}{\knots} \times \dfrac{\SI{0,5144}{m/s}}{\SI{1}{\knots}} = \SI{6,17}{m/s}$
...
Un traversier navigue à $\SI{18}{\knots}$ lorsque le capitaine ordonne l'arrêt des machines. La décélération est de $\SI{0,01}{m/s^2}$.
...
\end{tikzpicture}
\end{center}
Un traversier quitte le quai et atteint sa vitesse de croisi\`ere de $\SI{18}{\knots}$ apr\`es avoir parcouru $\SI{500}{m}$. Calculez :
\begin{enumerate}[label=\alph*)]
...
\begin{itemize}
    ...
    \item $v_f = \SI{18}{\knots} \times \dfrac{\SI{0,5144}{m/s}}{\SI{1}{\knots}} = \SI{9,26}{m/s}$
...
Un cargo navigue en MRU \`a $v_C = \SI{10}{\knots}$ ($\SI{5,14}{m/s}$) vers l'est. Un patrouilleur de la Garde c\^oti\`ere, initialement au repos \`a $d = \SI{2}{km}$ derri\`ere le cargo, d\'emarre avec une acc\'el\'eration constante de $a_P = \SI{0,15}{m/s^2}$.
...
Conversion : $v_P = 30,2 \times \dfrac{1}{0,5144} = \SI{58,7}{\knots}$
...
\end{tikzpicture}
\end{center}
Deux navires naviguent l'un vers l'autre dans un chenal \'etroit. Le navire A (cargo) se d\'eplace vers l'est \`a $\SI{10}{\knots}$. Le navire B (p\'etrolier) se d\'eplace vers l'ouest \`a $\SI{8}{\knots}$. Ils sont initialement s\'epar\'es de $\SI{3200}{m}$.
...
\begin{align*}
v_{A,i} &= \SI{+10}{\knots} \times \frac{\SI{0,5144}{m/s}}{\SI{1}{\knots}} = \SI{+5,14}{m/s} \quad \text{(vers l'est)} \\[0.2cm]
v_{B,i} &= \SI{-8}{\knots} \times \frac{\SI{0,5144}{m/s}}{\SI{1}{\knots}} = \SI{-4,12}{m/s} \quad \text{(vers l'ouest)}
\end{align*}
...
\begin{align*}
v_A &= \SI{0,43}{m/s} \times \frac{\SI{1}{\knots}}{\SI{0,5144}{m/s}} = \SI{0,8}{\knots} \\
v_B &= \SI{-1,17}{m/s} \times \frac{\SI{1}{\knots}}{\SI{0,5144}{m/s}} = \SI{-2,3}{\knots}
\end{align*}

\textbf{Vitesse relative d'impact :} 
\[ |v_A - v_B| = |\SI{0,43}{m/s} - (\SI{-1,17}{m/s})| = \SI{1,60}{m/s} = \SI{3,1}{\knots} \]

\begin{attention}
Cette collision \`a basse vitesse ($\sim\SI{3}{\knots}$) est comparable \`a un contact lors d'une man\oe{}uvre d'accostage rat\'ee. Les d\'eg\^ats seraient limit\'es, mais l'incident reste s\'erieux. Avec seulement $\SI{100}{m}$ de plus entre les navires au d\'epart, la collision aurait \'et\'e \'evit\'ee!
\end{attention}
\end{exemple}