...
Les navires se déplacent vers l'est à $\SI{1,8}{m/s}$ ($\approx \SI{3,5}{n\oe uds}$).
...
\begin{definition}[title=Cas particulier : cible au repos ($v_{2i} = 0$)]
...
\begin{align*}
v_f &= \sqrt{v_{fx}^2 + v_{fy}^2} = \sqrt{3{,}2^2 + 1{,}2^2} = \SI{3,4}{m/s} \approx \SI{6,6}{n\oe uds} \\
\theta &= \arctan\left(\frac{v_{fy}}{v_{fx}}\right) = \arctan\left(\frac{1{,}2}{3{,}2}\right) = 20{,}6° \text{ nord de l'est}
\end{align*}

L'ensemble se déplace à $\SI{3,4}{m/s}$ dans la direction $\text{N}69°\text{E}$ (cap $\approx 069°$).
\end{exemple}

\begin{pratiqueautonome}
Deux bateaux de pêche entrent en collision en eau libre :
\begin{itemize}
    \item Bateau A : $m_A = \SI{15}{tonnes}$, vitesse $\SI{5}{m/s}$ vers le nord (cap 000°)
    \item Bateau B : $m_B = \SI{10}{tonnes}$, vitesse $\SI{8}{m/s}$ vers l'est (cap 090°)
\end{itemize}
...
\reponsepratique{a) $v_{fx} = \SI{3,2}{m/s}$, $v_{fy} = \SI{3,0}{m/s}$ \quad b) $v_f = \SI{4,4}{m/s}$ \quad c) $\theta = 43°$ est du nord, cap $\approx 043°$ \quad d) $K_i = \SI{508}{kJ}$, $K_f = \SI{242}{kJ}$, perte $\approx 52\%$}
\end{pratiqueautonome}
